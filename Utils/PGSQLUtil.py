...
class PGSQLUtil:
    ...
    PostgreSQL工具类
    """
    def __init__(self, host=config["database"]['host'], user=config["database"]['user'], password=config["database"]['password'], database=config["database"]['database'], port=5432, dsn=None, connection_factory=None, cursor_factory=None, **kwargs):
        """构造函数"""


        self.__database = database
        self.__conn = psycopg2.connect(host=host, user=user, password=password, database=self.__database, port=str(port),
...
if __name__ == "__main__":
    pgsqlUtil = PGSQLUtil(host="127.0.0.1", user="postgres", password="postgres", database="dugking")
    ...
    for privilege in privileges:
        print(privilege)
    ## 查询表数据
    result = pgsqlUtil.execute("SELECT * FROM User")
    for i in result:
        print(i)
 
    # result = pgsqlUtil.get_table_fields("t_user")
    result = pgsqlUtil.table_metadata("User")
    for i in result:
        print(i)